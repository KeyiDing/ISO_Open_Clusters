...
def run_isochrones(row,name,base):
    ...
    # extinctionV = row['reddening[mag][stilism]'].iloc[0] * 3.1
    # extinctionV_err = row['reddening_uncertainty_max[mag][stilism]'].iloc[0] * 3.1
    # if extinctionV == 0:
    #     extinctionV = 0.001 * 3.1
    # if extinctionV_err == 0:
    #     extinctionV_err = 0.001 * 3.1
    # params_iso = {'parallax':(plx+zpt, row["parallax_error"].iloc[0]),'AV':(extinctionV,extinctionV_err)}
    ...
    #always use Gaia_G
    bands = ['Gaia_G_DR2Rev']
    mags_iso = {'Gaia_G_DR2Rev':(row['gmag'].iloc[0],np.max([0.0003,row['g_flux-mag_err'].iloc[0]]))}
    ...
    if sdss_flag == 0 and skymapper_flag == 0:
        if row['v_psf'].isna().iloc[0]==False and row['e_v_psf'].isna().iloc[0]==False:
            skymapper_bands.append('SkyMapper_v')
            skymapper_mags_iso['SkyMapper_v'] = (row['v_psf'].iloc[0], row['e_v_psf'].iloc[0])
        if row['g_psf'].isna().iloc[0]==False and row['e_g_psf'].isna().iloc[0]==False:
            skymapper_bands.append('SkyMapper_g')
            skymapper_mags_iso['SkyMapper_g'] = (row['g_psf'].iloc[0], row['e_g_psf'].iloc[0])
        if row['r_psf'].isna().iloc[0]==False and row['e_r_psf'].isna().iloc[0]==False:
            skymapper_bands.append('SkyMapper_r')
            skymapper_mags_iso['SkyMapper_r'] = (row['r_psf'].iloc[0], row['e_r_psf'].iloc[0])
        if row['i_psf'].isna().iloc[0]==False and row['e_i_psf'].isna().iloc[0]==False:
            skymapper_bands.append('SkyMapper_i')
            skymapper_mags_iso['SkyMapper_i'] = (row['i_psf'].iloc[0], row['e_i_psf'].iloc[0])
        if row['z_psf'].isna().iloc[0]==False and row['e_z_psf'].isna().iloc[0]==False:
            skymapper_bands.append('SkyMapper_z')
            skymapper_mags_iso['SkyMapper_z'] = (row['z_psf'].iloc[0], row['e_z_psf'].iloc[0])

        if row['psfMag_gSDSS'].isna().iloc[0]==False and row['psfMagErr_gSDSS'].isna().iloc[0]==False:
            sdss_bands.append('SDSS_g')
            sdss_mags_iso['SDSS_g'] = (row['psfMag_gSDSS'].iloc[0], row['psfMagErr_gSDSS'].iloc[0])
        if row['psfMag_rSDSS'].isna().iloc[0]==False and row['psfMagErr_rSDSS'].isna().iloc[0]==False:
            sdss_bands.append('SDSS_r')
            sdss_mags_iso['SDSS_r'] = (row['psfMag_rSDSS'].iloc[0], row['psfMagErr_rSDSS'].iloc[0])
        if row['psfMag_iSDSS'].isna().iloc[0]==False and row['psfMagErr_iSDSS'].isna().iloc[0]==False:
            sdss_bands.append('SDSS_i')
            sdss_mags_iso['SDSS_i'] = (row['psfMag_iSDSS'].iloc[0], row['psfMagErr_iSDSS'].iloc[0])
        if row['psfMag_zSDSS'].isna().iloc[0]==False and row['psfMagErr_zSDSS'].isna().iloc[0]==False:
            sdss_bands.append('SDSS_z')
            sdss_mags_iso['SDSS_z'] = (row['psfMag_zSDSS'].iloc[0], row['psfMagErr_zSDSS'].iloc[0])
        
        
        # no PAN-STARRs for M67  
        # if row['gMeanPSFMag'].isna().iloc[0] == False and row['gMeanPSFMagErr'].isna().iloc[0] == False and row['gMeanPSFMag'].iloc[0]>14.5:
        #     pan_bands.append("PS_g")
        #     pan_mags_iso["PS_g"] = (row["gMeanPSFMag"].iloc[0],row["gMeanPSFMagErr"].iloc[0])
        # if row['rMeanPSFMag'].isna().iloc[0] == False and row['rMeanPSFMagErr'].isna().iloc[0] == False and row['rMeanPSFMag'].iloc[0]>15:
        #     pan_bands.append("PS_r")
        #     pan_mags_iso["PS_r"] = (row["rMeanPSFMag"].iloc[0],row["rMeanPSFMagErr"].iloc[0])
        # if row['iMeanPSFMag'].isna().iloc[0] == False and row['iMeanPSFMagErr'].isna().iloc[0] == False and row['iMeanPSFMag'].iloc[0]>15:
        #     pan_bands.append("PS_i")
        #     pan_mags_iso["PS_i"] = (row["iMeanPSFMag"].iloc[0],row["iMeanPSFMagErr"].iloc[0])
        # if row['zMeanPSFMag'].isna().iloc[0] == False and row['zMeanPSFMagErr'].isna().iloc[0] == False and row['zMeanPSFMag'].iloc[0]>14:
        #     pan_bands.append("PS_z")
        #     pan_mags_iso["PS_z"] = (row["zMeanPSFMag"].iloc[0],row["zMeanPSFMagErr"].iloc[0])
        # if row['yMeanPSFMag'].isna().iloc[0] == False and row['yMeanPSFMagErr'].isna().iloc[0] == False and row['yMeanPSFMag'].iloc[0]>13:
        #     pan_bands.append("PS_y")
        #     pan_mags_iso["PS_y"] = (row["yMeanPSFMag"].iloc[0],row["yMeanPSFMagErr"].iloc[0])
            
        if len(sdss_bands) >= len(skymapper_bands) and len(sdss_bands) >= len(pan_bands):
            bands += sdss_bands
            mags_iso = {**mags_iso,**sdss_mags_iso}
        elif len(skymapper_bands) >= len(sdss_bands) and len(skymapper_bands) >= len(pan_bands):
            bands += skymapper_bands
            mags_iso = {**mags_iso,**skymapper_mags_iso}
        elif len(pan_bands) >= len(skymapper_bands) and len(pan_bands) >= len(sdss_bands):
            bands += pan_bands
            mags_iso = {**mags_iso,**pan_mags_iso}
    ...
    # model1._bounds['AV'] = (np.max(0,extinctionV-3*extinctionV_err), extinctionV+3*extinctionV_err)
    # model1.set_prior(AV=GaussianPrior(mean=extinctionV, sigma=extinctionV_err))
    # model1.set_prior(AV=FlatPrior((np.max([0,extinctionV-3*extinctionV_err]), extinctionV+3*extinctionV_err)))
    # MeanDistance = 1000 / (plx+zpt)
    # HighDistance = 1000 / ((plx+zpt)-3*params_iso['parallax'][1])
    # LowDistance = 1000 / ((plx+zpt)+3*params_iso['parallax'][1])
